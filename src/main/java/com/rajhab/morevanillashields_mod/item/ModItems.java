            ...
            () -> new ShieldItem(new Item.Properties().defaultDurability(250).tab(CreativeModeTab.TAB_COMBAT)) {
                ...
                public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
                    ...
                    if (ShieldConfig.ENABLE_TOOLTIPS.get()) {
                        if(ShieldConfig.ENABLE_PARTICLES.get()) {
                            if (Screen.hasShiftDown()) {
                                components.add(new TranslatableComponent("item.moditems.redstone_shield").append(String.valueOf(ShieldConfig.REDSTONE_SHIELD_DURABILITY.get())).append(new TranslatableComponent("item.moditems.redstone_shield.particles_enabled")).withStyle(ChatFormatting.DARK_AQUA));
                            } else {
                                components.add(new TranslatableComponent("item.moditems.shift").withStyle(ChatFormatting.LIGHT_PURPLE));
                            }
                            ...
                            if (Screen.hasShiftDown()) {
                                components.add(new TranslatableComponent("item.moditems.redstone_shield").append(String.valueOf(ShieldConfig.REDSTONE_SHIELD_DURABILITY.get())).append(new TranslatableComponent("item.moditems.redstone_shield.particles_disabled")).withStyle(ChatFormatting.DARK_AQUA));
                            } else {
                                components.add(new TranslatableComponent("item.moditems.shift").withStyle(ChatFormatting.LIGHT_PURPLE));
                            }
